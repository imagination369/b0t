    ...
    },
    {
      ...
      modules: [
        ...
        { name: 'gohighlevel', functions: [
            { name: 'apiRequest', description: 'Generic API request for any GHL endpoint (maximum flexibility)', signature: 'apiRequest({ method, endpoint, locationId?, data?, params?, headers? })' },
            { name: 'createContact', description: 'Create contact', signature: 'createContact(locationId, { firstName?, lastName?, email?, phone?, tags?, customFields? })' },
            { name: 'getContact', description: 'Get contact by ID', signature: 'getContact(locationId, contactId)' },
            { name: 'updateContact', description: 'Update contact', signature: 'updateContact(locationId, contactId, { firstName?, lastName?, email?, phone?, tags?, customFields? })' },
            { name: 'searchContacts', description: 'Search/list contacts', signature: 'searchContacts(locationId, { query?, email?, phone?, limit?, skip? })' },
            { name: 'deleteContact', description: 'Delete contact', signature: 'deleteContact(locationId, contactId)' },
            { name: 'createOpportunity', description: 'Create opportunity', signature: 'createOpportunity(locationId, { pipelineId, pipelineStageId, name, monetaryValue?, contactId?, status?, assignedTo? })' },
            { name: 'updateOpportunity', description: 'Update opportunity', signature: 'updateOpportunity(locationId, opportunityId, { pipelineStageId?, name?, monetaryValue?, status?, assignedTo? })' },
            { name: 'listOpportunities', description: 'List opportunities', signature: 'listOpportunities(locationId, { pipelineId?, pipelineStageId?, assignedTo?, limit?, skip? })' },
            { name: 'sendSMS', description: 'Send SMS to contact', signature: 'sendSMS(locationId, contactId, message, { mediaUrl? })' },
            { name: 'sendEmail', description: 'Send email to contact', signature: 'sendEmail(locationId, contactId, { subject, body, fromEmail?, fromName?, attachments? })' },
            { name: 'createAppointment', description: 'Create appointment', signature: 'createAppointment(locationId, { calendarId, contactId, startTime, endTime, title, notes?, assignedTo? })' },
            { name: 'listAppointments', description: 'List appointments', signature: 'listAppointments(locationId, { contactId?, calendarId?, startDate?, endDate?, limit? })' },
            { name: 'getTags', description: 'Get all tags for location', signature: 'getTags(locationId)' },
            { name: 'addTagToContact', description: 'Add tag to contact', signature: 'addTagToContact(locationId, contactId, tagId)' },
            { name: 'removeTagFromContact', description: 'Remove tag from contact', signature: 'removeTagFromContact(locationId, contactId, tagId)' },
            { name: 'updateCustomField', description: 'Update custom field value', signature: 'updateCustomField(locationId, contactId, fieldId, value)' },
            { name: 'triggerWorkflow', description: 'Trigger workflow for contact', signature: 'triggerWorkflow(locationId, workflowId, contactId, eventData?)' },
          ]},
